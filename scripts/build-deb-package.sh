...
# Look for superhuman icons extracted from the exe
# Icon naming from icotool: superhuman_N_WxHx32.png
...
for size in "${!icon_files[@]}"; do
	...
	icon_source_path="$work_dir/superhuman_${icon_files[$size]}_${size}x${size}x32.png"
	if [[ -f $icon_source_path ]]; then
		echo "Installing ${size}x${size} icon..."
		install -Dm 644 "$icon_source_path" "$icon_dir/superhuman.png" || exit 1
	else
...
cat > "$install_dir/share/applications/superhuman.desktop" << EOF
[Desktop Entry]
Name=Superhuman
Comment=The fastest email experience ever made
Exec=/usr/bin/superhuman %u
Icon=superhuman
Type=Application
Terminal=false
...
cat > "$install_dir/bin/superhuman" << EOF
#!/usr/bin/env bash
...
if ! check_display; then
	...
	echo 'Error: Superhuman requires a graphical desktop environment.' >&2
...
if [[ -f \$local_electron_path ]]; then
	...
	if command -v electron &> /dev/null; then
		...
		if command -v zenity &> /dev/null; then
			zenity --error \
				--text='Superhuman cannot start because the Electron framework is missing.'
		elif command -v kdialog &> /dev/null; then
			kdialog --error \
				'Superhuman cannot start because the Electron framework is missing.'
		fi
		exit 1
	fi
fi
...
log_message '--- Superhuman Launcher End ---'
exit \$exit_code
EOF
chmod +x "$install_dir/bin/superhuman" || exit 1
...
Description: $description
 ...
 This package provides the desktop interface for Superhuman.
 .
